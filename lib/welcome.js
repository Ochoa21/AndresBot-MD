...
const genSVG = async ({
  ...
  const el = {
    code: ['#_1661899539392 > g:nth-child(6) > image', imageSetter, toBase64(await toImg(barcode(wid.replace(/[^0-9]/g, '')), 'webp'), 'sticker/webp')],
...
const render = async ({
  wid = '',
  pp = toBase64(readFileSync(join(src, 'callabo.webp')), 'stciker/webp'),
  name = '',
  title = '',
  text = '',
